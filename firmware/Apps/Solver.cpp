                               ...
                               int nb_eq, int nb_var, int *var_indices)
{
    ...
    for (int start = 0; start < 4 && !converged; start++)
    {
        ...
        for (int iter = 0; iter < MAX_ITER && !converged; iter++)
        {
            ...
            for (int i = 0; i < nb_eq; i++)
            {
                F[i] = eval_multi(left_tok[i], left_sz[i], x, nb_var, var_indices) - eval_multi(right_tok[i], right_sz[i], x, nb_var, var_indices);
                if (fabs(F[i]) > max_f)
                    max_f = fabs(F[i]);
            }
            ...
            for (int j = 0; j < nb_var; j++)
            {
                ...
                for (int i = 0; i < nb_eq; i++)
                {
                    double Fh = eval_multi(left_tok[i], left_sz[i], xh, nb_var, var_indices) - eval_multi(right_tok[i], right_sz[i], xh, nb_var, var_indices);
                    J[i][j] = (Fh - F[i]) / H;
                }
                free(xh);
            }
