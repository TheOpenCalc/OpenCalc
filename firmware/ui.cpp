...
void ili_init() {
    ili_reset();
    ili_cmd(0x01); sleep_ms(5);
    ili_cmd(0x28);
    ili_cmd(0x3A); ili_data(0x55);
    ili_cmd(0x36); ili_data(0x48);
    ili_cmd(0x11); sleep_ms(120);
    ili_cmd(0x29);
}

void fill_screen(uint16_t color) {
    ili_cmd(0x2A); ili_data(0); ili_data(0); ili_data(0); ili_data(239);
    ili_cmd(0x2B); ili_data(0); ili_data(0); ili_data(1); ili_data(63);
    ili_cmd(0x2C);
...
void fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    if ((x + w - 1) >= SCREEN_HEIGHT)  w = SCREEN_HEIGHT - x;
    if ((y + h - 1) >= SCREEN_WIDTH) h = SCREEN_WIDTH - y;
...
int i = 0 ;
    while(c[i]!='\0'){
    if (c[i] < 32 || c [i]> 126) return; // Caractères imprimables uniquement
